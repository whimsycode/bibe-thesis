...
#try number 1
flags.DEFINE_string('video', 'rtsp://admin:Tatlong-B1b3@172.16.0.99/H264?ch=1&subtype=0', 'path to input video or set to 0 for webcam')  
#original:
...
def addDatabase(numPeople):
    ...
    conn.execute("CREATE TABLE IF NOT EXISTS peopleCount2(totalPeople)")
    a = "INSERT INTO peopleCount2 VALUES ({0})".format(numPeople, )
    conn.execute(a)
    conn.commit()
    conn.close()
    ...
    while True:
        ...
        if FLAGS.count:
            ...
            for key, value in counted_classes.items():
                pcount2 = value
                addDatabase(pcount2) #This is new.... Azure SQL
                #print("Number of {}s: {}".format(key, pcount2))
                #return pcount2
                '''
                print("Number of {}s: {}".format(key, pcount2))
                res = requests.post('http://127.0.0.1:5000/', json={"numofPeople2":pcount2})
                if res.ok:
